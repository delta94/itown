...
class CheckoutScreen extends React.Component{
    ...
    getSubmitData(){
        ...
        const {notes,pmMethod} = this.state
            ...
            for( let pm in payments ){
                ...
                pmJsx.push(
                    <View key={pm}>
                        ...
                        { pmMethod === pm && null != payments[pm]['desc'] && payments[pm]['desc'] != '' && <HTML html={payments[pm]['desc']}
    ...
    render(){
        ...
        const _self = this
        const {apColors} = this.props;
        const {notes,focus,payBtnText,waitingPayment,comSubmitted,paymentCompleted,customPMForm} = this.state
        const {submitting,submitted,isSuccess,url,bk_status} = this.props.booking;
        const {price_based} = this.props.booking.datas
        let notesFocus = {},submittedBtn = {}
